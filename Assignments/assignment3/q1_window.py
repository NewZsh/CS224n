...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for idx in range(window_size, sent_len + window_size):
            window = list()
            for word in sentence[idx - window_size:idx + window_size + 1]:
                window += word
            windowed_data.append((window, labels[idx - window_size]))    
	    ### END YOUR CODE
